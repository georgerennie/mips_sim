...
mips_trap_t mips_core_cycle(mips_core_t* core) {
	...
	if (next_state.trap_stage > 0) { next_state.trap_stage += 1; }

	// Instruction Fetch
	log_assert(core->state.pc % 4 == 0); // Check that the pc is word aligned
	if (!(core->stalls[0] || core->trap_stage > 0)) {
	...
	if (!(core->stalls[1] || core->trap_stage > 1)) {
		decode_result_t dec    = decode_instruction(&core->state, core->decoded_instruction);
		next_state.exec_bundle = dec.exec;
		if (dec.trap) {
			next_state.trap |= dec.trap;
			next_state.trap_stage = 2;
			// TODO: undo increment in PC from following instruction being in IF
		}
	}

	// Execute
	if (!(core->stalls[2] || core->trap_stage > 2)) {
